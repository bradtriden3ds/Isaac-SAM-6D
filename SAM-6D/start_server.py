...
def run_sam6d(
    ...
    input_data, img, whole_pts, model_points, detections = get_test_data(
        rgb_path, depth_path, cam_path, cad_path, results, 
        pose_estimation_test_config
    )
    ...
    if visualize:
        ...
        for idx in range(len(detections)):
            ...
            save_path = os.path.join(f"{output_dir}/sam6d_results", 'vis_pem_server4'+str(idx)+'.png')
    ...
    logging.warning({"message": "SAM-6D inference completed.", "image_segmentation_time_cost": image_segmentation_time_cost, "total_time": total_time})

    return {
        "rotation": filtered_pred_rot.tolist(),
